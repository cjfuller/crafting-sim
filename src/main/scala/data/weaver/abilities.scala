    ...
    otherEffect = Effect(
      ...
      postFn = (e, prev, curr, ability) => {
        ...
        if (prev.quality < curr.quality) {
          (addInnerQuietBuffs(curr, 1), Modifier.NoModifier)
        } else {
          (curr, Modifier.NoModifier)
        }
      },
    ...
    otherEffect = Effect(
      ...
      postFn = (e, prev, curr, ability) => {
        if (prev.quality < curr.quality) {
          ...
          val numToLose = countInnerQuiet(curr) / 2 // Rounds towards 0
          (addInnerQuietBuffs(curr, -numToLose), Modifier.NoModifier)
        }
      },
      duration = 0
    )
